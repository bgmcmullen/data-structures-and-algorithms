...
const screenForNames = (arr) => arr.filter(name => /^(?:Mr\.|Mrs\.|Ms\.|Dr\.)\s[a-zA-Z]+(?:\s[a-zA-Z]+)?$/.test(name));
...
const toTitleCase = (arr) => arr.map(element => element[0].toUpperCase() + element.substring(1, element.length));
...
const detectTicTacToeWin = (board) => {
  ...
  const diag2 = [board[2][0], board[1][1], board[2][0]];
  const allLines = [row1, row2, row3, col1, col2, col3, diag1, diag2];

  function checkForWin(line){
    if(line.join('') === 'XXX' || line.join('') === 'OOO')
      result = true;
  }
  allLines.forEach(line => checkForWin(line));
  return result;
};
