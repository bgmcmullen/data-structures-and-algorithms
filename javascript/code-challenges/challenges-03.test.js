...
const containsAnd = (arr) => arr.filter((element) => /and/.test(element));
...
const evenOddNumericValues = (arr) => arr.filter((element) => (!isNaN(element)  && typeof element === 'number')).map((element) => (element % 2) ? 'odd' : 'even');
