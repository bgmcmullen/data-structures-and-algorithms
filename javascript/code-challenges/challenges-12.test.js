...
function transformToLis(obj){
  ...
  for (let key in obj) {
    arrayOfListItems.push(`<li>${key}: ${obj[key]}</li>`);
  }
...
const count = (target, input) => {
  ...
  let targetCount = 0;
  input.forEach(element => element.forEach(element => {
    if(element === target)
      targetCount++;
  }));
...
const totalSum = (input) => {
  return input.reduce((accumulator, currentValue) => accumulator + currentValue.reduce((innerAccumulator, innerCurrentValue) => innerAccumulator + parseInt(innerCurrentValue), 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  const divisibleByFive = input.map(element => element.filter(innerElement => (!(innerElement % 5) && typeof innerElement === 'number')));

  return divisibleByFive.map(element => element.map(innerElement => (2 ** innerElement)));
};
...
let findMaleAndFemale = (data) => {
  const maleAndFemale = data.filter(element => element.gender === 'female' || element.gender === 'male');

  return maleAndFemale.slice(1).reduce((accumulator, currentValue) => accumulator += ` and ${currentValue.name}`, maleAndFemale[0].name);
};
...
let findShortest = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    if (Number(data[i].height) < Number(data[output].height))
      output = i;
  }
