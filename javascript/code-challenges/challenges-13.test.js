...
const sortByChildren = (charArray) => {
  const output = charArray.sort((a, b) => {
...
const isCapitalized = (str) => {
  return str.split(' ').filter(element => /\b[A-Z][a-z]*\b/.test(element)).map(text => text.replace(/[_\W]+/g, ""));
};
...
const citiesAtoJ = (arr) => {
  return arr.filter(element => /^[A-J]/i.test(element)).map(text => text.replace(/[_\W]+/g, ""));
};
