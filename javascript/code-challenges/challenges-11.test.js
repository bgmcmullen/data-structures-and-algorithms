...
const longestString = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].length > arr[output].length)
      output = i;
  }
  return output;
};
...
const findHappiness = (arr) => arr.filter(element => typeof element === 'string' && element.includes(':)'));
...
const standardizePhoneNumbers = (arr) => {
  const output = arr.map(element => {
    ...
    const onlyNumbersArray = elementArray.filter(character => (!isNaN(parseInt(character))));
    return onlyNumbersArray.join('');
