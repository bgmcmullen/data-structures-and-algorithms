...
const raisedToTheThird = (arr) => {
  let outputArr = [];
  arr.forEach((element) => {
    outputArr.push(Math.pow(element, 3));
  })
  return outputArr;
};
...
const addOne = (arr) => arr.map((element) => ++element);
...
const forLoopTwoToThe = (arr) => {
  ...
  for(let index in arr){
    outputArr.push(Math.pow(2, arr[index]));
  }
  return outputArr;
};
...
const forEachTwoToThe = (arr) => {
  let outputArr = [];
  arr.forEach(element => outputArr.push(Math.pow(2, element)));
  return outputArr;
};
...
const charCode = (arr) => arr.maps(element => element.charCodeAt());
